...
/* Defining schema for login details */
...
/* Defining schema for note storage */
...
/* Login data sent to server from client */
...
/* New registration data sentn to server from client */
app.post("/register", async (req, res) => {
  ...
  /* Registration data checked if already present otherwise stored */
  try {
    const user = await login_collection.findOne({ email: email });
    if (user) {
...
/* Password reset data sent to server from user */
...
app.post("/dashboard/account-delete", async (req, res) => {
  ...
  try {
    ...
    /* Commit transaction if succesfully deleted */
